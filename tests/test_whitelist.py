...
class TestWhitelist(TestCase):
    ...
    def test_validate(self):
        ...
        wlist = {'wl': [1000], 'mz': ['$ARGS_VAR_X:test', '$HEADERS_VAR:lol']}
        errors, warnings = whitelist.validate(wlist)
        self.assertEqual(errors, [])
        self.assertEqual(warnings, [])
        ...
        wlist = {'wl': [1000], 'mz': ['$ARGS_VAR_X:lol', 'ARGS']}
        errors, warnings = whitelist.validate(wlist)
        ...
        wlist = {'wl': [1000], 'mz': ['ARGS', '$ARGS_VAR_X:pouet', 'NAME']}
        errors, warnings = whitelist.validate(wlist)
        self.assertEqual(errors, ['You can not use regexp matchzone with non-regexp one'])
    ...
    def test_explain(self):
        ...
        self.assertEqual(whitelist.explain(wlist), 'Whitelist the rule 1000 if matching in $ARGS_VAR:foo in $URL:/bar.')
